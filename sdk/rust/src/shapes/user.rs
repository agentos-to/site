...
pub static USER: Lazy<ShapeDef> = Lazy::new(|| ShapeDef {
    ...
    prior_art: vec![
        ...
        PriorArtDef { source: "schema.org/Audience".into(), url: Some("https://schema.org/Audience".into()), notes: Some("Loose fit. schema.org has no \"OS user\" concept — user accounts are product-specific. We model it explicitly because AgentOS is the product, and the user-as-pref-carrier is load-bearing.".into()) },
        PriorArtDef { source: "macOS NSUserDefaults".into(), url: Some("https://developer.apple.com/documentation/foundation/nsuserdefaults".into()), notes: Some("The Apple model: per-user preference store, keyed by the OS account. Our `pref:ui` / `pref:system` blobs play the same role, but live in the graph (queryable, multi-process safe) rather than a plist file.".into()) },
    ],
