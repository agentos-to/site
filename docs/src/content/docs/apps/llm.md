...
- `model` — `"opus"`, `"sonnet"`, `"haiku"`, or provider-specific (default `"sonnet"`)
- `tools` — tool refs in `"app.tool"` format (e.g., `["exa.search"]`)
...
Tool refs identify tools available to agents. Format: `"app.tool"` (e.g., `"exa.search"`, `"hackernews.search_posts"`).
...
All `llm` functions are async. Apps that use them must be `async def`:
...
The engine detects async functions and runs them on an asyncio event loop. Sync functions continue to work as before — no migration required for apps that don't need agent workflows.

## Checkpoint — resume multi-phase workflows

Long-running apps (multi-agent proposal writing, research pipelines) can save state after each completed phase and resume on restart.
...
Checkpoint is opt-in. Apps that don't call `save()` have no checkpoint file. The file is `.checkpoint.json` in the app's output directory.

## Provider architecture

LLM providers (claude, anthropic-api, openrouter, ollama) are apps that declare `@provides(llm)` with feature flags:
