...
**Entities first, apps second.** The graph covers 90% of what an agent needs. Apps are the escape hatch for what the graph can't provide — searching the web, sending a message, calling an external API. If an agent reaches for an app when an entity query would have worked, the documentation failed, not the agent.
