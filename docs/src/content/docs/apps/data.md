---
title: "Data & Storage"
description: "Apps can persist state across runs using two reserved keys on their graph node:"
---

## Sandbox storage

Apps can persist state across runs using two reserved keys on their graph node:

- **`cache`** — regeneratable state (discovered endpoints, scraped tokens). Can be cleared at any time; the app re-discovers on next run.
- **`data`** — persistent state (settings, preferences, sync timestamps). Survives cache clears.

If losing it requires user action to recover (re-entering a setting), it's data. If the app can regenerate it, it's cache.
...
- `__cache__` — merged into the app node's cache
- `__data__` — merged into the app node's data
...
A third reserved key, `__secrets__`, handles importing secrets from external sources (password managers, payment info, identity documents, etc.) into the vault. The `__secrets__` handler is pure vault CRUD — it writes credential rows and strips the key. It does **not** create graph entities or links; entity creation happens through the normal adapter pipeline processing `__result__`. The two systems are joined by `(issuer, identifier)`.
...
Leading by example: `apps/media/goodreads/public_graph.py` (GraphQL endpoint discovery cached via `__cache__`).
...
When a `command:` argument or `working_dir:` looks like a relative file path, it is resolved relative to the app folder. Prefer `./my_script.py` over machine-specific absolute paths.

If you need advanced command, steps, or crypto behavior, copy from an existing app.
