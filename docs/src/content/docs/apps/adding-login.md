---
title: "How to add login to an app"
description: "Step-by-step walkthrough for wiring a new platform's login flow into an app. Copy-pasteable templates, the three credential resolution paths, common gotchas."
---

Your app talks to a platform that requires a login. This page
walks through adding a `login` tool to that app so the engine
...
you just want to copy an existing app and adapt it.
...
1. Open the platform's login page with `browse-capture.py`.
...
platforms like Amazon return anti-bot error pages when you hit
...
async def login(*, email: str = "", password: str = "", **params) -> dict:
    ...
    # Replace with whatever step 2 revealed. Cookies the platform
    ...
    if resp["status"] != 200:
        return app_error(f"login failed: HTTP {resp['status']}",
                         code="AuthFailed")
...
- **Cookies self-persist via the ambient Jar.** Any `Set-Cookie`
  the platform returns during the handshake rides back through
...
If your app previously had users paste credentials into
...
Symmetric with `login`. The platform-side revoke is the part the
...
agentos call apps '{"op":"run","params":{"app":"example","tool":"login"}}'
# ... test a few authed tools ...
agentos call apps '{"op":"run","params":{"app":"example","tool":"logout"}}'
```
...
2. `credentials.retrieve(".example.com", ...)` dispatches every
   installed `@provides(login_credentials)` app.
...
7. When the user's done, `apps.run({app:"example",tool:"logout"})`
   dispatches your `logout` op (platform-side revoke), then the
...
Clean shutdown, **one call, tokens dead at the platform, local
cache clear.**
...
2. **`credentials.retrieve(domain, required)`.** Matchmakes every
   ...
   app always tries first — it reads the local vault in ~ms and
...
Three apps in the public repo, each a different auth shape.
Copy the one closest to yours.

| App | Auth shape | What to learn from it |
|---|---|---|
| [`apps/fitness/austin-boulder-project/abp.py`](https://github.com/agentos-to/apps/tree/main/fitness/austin-boulder-project) | Cognito / Amplify | OAuth-provider handshake + portal session follow-up |
| [`apps/web/exa/exa.py`](https://github.com/agentos-to/apps/tree/main/web/exa) | NextAuth + email OTP | Two-step flow (`send_login_code` + `verify_login_code`) |
| [`apps/media/goodreads/goodreads_web.py`](https://github.com/agentos-to/apps/tree/main/media/goodreads) | Plain form POST | Cookie-only, no tokens, identity scraped from `/user/edit` |
...
(you'll see it as an `app_error` response) — unlock the
...
### My platform uses a non-email username
...
platform uses as its login ID (username, phone number, member
...
2. **Cookies landed on a different domain than the connection
   ...
   the platform sets cookies on `.api.example.com`, the lookup
...
async def logout(**params) -> dict:
    """Tell the platform this session is over."""
    ...
    resp = await client.post(REVOKE_URL, json={"token": token},
...
**Apps without a `logout` have no engine-side teardown path.**
The cache-only `clear_session` variant
...
(see [`fix-auth/credential-shape-and-vault.md`](https://github.com/agentos-to/core/blob/main/_roadmap/p2/fix-auth/credential-shape-and-vault.md)),
