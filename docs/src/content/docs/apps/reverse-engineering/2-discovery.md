...
> **Why not Playwright?** Playwright's bundled Chromium has a detectable TLS fingerprint. Sites like Amazon and Cloudflare-protected sites reject it. CDP to a real browser produces authentic fingerprints and uses existing sessions. See [Transport](./1-transport.md#cdp-detection-signals--why-playwright-gets-caught).
...
See `apps/media/goodreads/public_graph.py` functions `load_book_page()` and
...
- Goodreads: `apps/media/goodreads/public_graph.py` `discover_from_bundle()` — extracts Prod AppSync config from `_app` chunk
- Austin Boulder Project: `apps/fitness/austin-boulder-project/abp.py` — API key and namespace from Tilefive bundle
...
- **Errors with stack traces** often expose the server file path
  (`/var/task/api/Customer/booking.js:175992:41`) — gives you the backend's
...
See `apps/media/goodreads/public_graph.py` for the full set of proven GraphQL queries
...
| App | Discovery technique | Reference |
|---|---|---|
| `apps/web/exa/` | JS bundle scanning for custom `/api/verify-otp` endpoint + Navigation API interception for token format + reading NextAuth source for server-side verification logic | `exa.py`, [nextauth.md](./3-auth/nextauth.md) |
| `apps/media/goodreads/` | Next.js Apollo cache + AppSync GraphQL + JS bundle scanning | `public_graph.py` |
| `apps/fitness/austin-boulder-project/` | JS bundle config extraction (API key + namespace) | `abp.py` |
| `apps/ai/claude/` | Session cookie capture via Playwright | `claude-login.py` |
| `apps/logistics/united/` | CDP Fetch interception + React fiber traversal for in-memory bundle offers | `requirements.md` |
...
- You need to *modify* a request (rare for apps)
...
Build these invariants into the app so a buggy caller can't
...
signal that the app will need explicit wait logic.
