...
description: "How to build apps against web platforms that don't have public APIs — extracting data surfaces, auth flows, and content, then packaging them as reliable AgentOS apps."
---

How to build apps against web platforms that don't have public APIs. This is the methodology for extracting data surfaces, auth flows, and content from any website — then packaging them as reliable AgentOS apps.
...
|-------|---------------|-----------------|
| [1. Transport](1-transport.md) | TLS fingerprinting, WAF bypass, Playwright stealth, HTTP/2 | The platform blocks automated requests |
...
| [6. Desktop Apps](6-desktop-apps/overview.mdx) | Electron asar extraction, native app IPC, plist configs | Local desktop software without web APIs |
| [7. MCP Servers](7-mcp.md) | Wrapping existing MCP servers as apps | When someone already built an MCP server |

## Core principle

**CDP discovers, `agentos.http` runs.**

Use **`browse capture`** (CDP to a real browser) to investigate — navigate pages, capture every network request with full headers and response bodies, inspect cookies. Then implement what you learned as Python + `agentos.client` in the app. No browser at runtime.
...
Browser-grade headers (UA, `Sec-CH-UA*`, `Sec-Fetch-*`, `Accept-*`) come from the connection's `client=` value — `"browser"` for full navigate bundle, `"fetch"` for XHR-style. The Rust engine's TLS emulation fingerprints as Chrome; the SDK's Python client-hint strings match it. Apps don't compose headers per-request.
...
4. **Replay** — reproduce what you found with `agentos.client` + a `client="browser"` or `"fetch"` connection. Cookies ride automatically via the ambient Jar. Test with `agentos browse request <app> <url>`.
5. **Implement** — write the app tool in Python with `agentos.client`. No browser dependency at runtime.
6. **Test** — declare a `test:` block in the readme, run `agent-sdk validate`, and exercise each tool through `agentos call apps '{"op":"run",...}'`.

### Browse toolkit commands

| Command | What it does |
|---------|-------------|
| `agentos browse request <app> <url>` | Make an authenticated HTTP request (same TLS fingerprint as engine), show full headers, cookies, response |
| `agentos browse cookies <app>` | Cookie inventory — all cookies from all sources with timestamps and provenance |
| `agentos browse auth <app>` | Auth resolution trace — which provider won, identity, timing |
...
See [`core/bin/README.md`](https://github.com/agentos-to/core/blob/main/bin/README.md) in the engine repo for the full capture-tooling reference.
...
## Starting a new reverse-engineered app

```bash
agent-sdk new-app my-platform

# Then start investigating:
# 1. Launch Brave with --remote-debugging-port=9222 and open the platform
# 2. bin/browse-capture.py to find API endpoints
# 3. Inspect the captured requests to understand the data flow
# 4. Document what you find in requirements.md
# 5. Implement with agentos.client in Python
```
...
| `apps/logistics/amazon/` | Deep anti-bot bypass (client hints, Siege encryption, session warming), session staleness (30-min TTL, CDP session warming), fallback CSS selector chains for resilient HTML parsing, AJAX endpoints for dynamic content, `SESSION_EXPIRED` provider retry convention, tiered cookie architecture. Full reference for [1-transport](1-transport.md) and [4-content](4-content.md). |
| `apps/web/exa/` | Full credential bootstrap: NextAuth email code → Playwright form submit → session cookies → API key extraction from dashboard API. Reference for [nextauth.md](3-auth/nextauth.md) |
| `apps/media/goodreads/` | Multi-tier discovery, Apollo cache extraction, auth boundary mapping, runtime config fallback |
| `apps/ai/claude/` | Cookie-based auth, Cloudflare stealth settings, API replay from browser session |
| `apps/fitness/austin-boulder-project/` | JS bundle config extraction, tenant-namespace auth |
