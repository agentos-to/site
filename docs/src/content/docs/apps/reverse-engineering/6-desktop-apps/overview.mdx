...
<CardGrid>
  <LinkCard title="Electron deep dive" href="/apps/reverse-engineering/6-desktop-apps/electron/" description="asar extraction, token files, CrossAppAuth, feature flags." />
</CardGrid>
...
AgentOS app gets `401`, the user needs to **open the desktop app** to refresh. Document this.
...
Tools bind with `@connection("api")` or `@connection("cache")`. A tool that can
dispatch to either uses `@connection(["api", "cache"])`; others (e.g.
`get_meeting` with full transcript) may be API-only if the cache doesn't
store transcripts.
...
### Pure-cache apps (WhatsApp, Copilot Money)
...
Then implement the app using those findings. The subagent does the tedious
search-and-document step; you do the clean integration.

---

## Checklist: New Desktop-App-Backed App
...
| 9 | Implement: same patterns as web apps — operations, adapters, error handling |

---

## Real-World Examples

| App | Discovery path | API + cache |
|-------|----------------|-------------|
| `apps/productivity/granola/` | supabase.json token, cache-v6.json entities, app.asar → get-entity-set/batch, grouping_key for meeting→thread link | Yes — api/cache/auto via `source` param |
| `apps/comms/whatsapp/` | ChatStorage.sqlite | Cache-only (no API) |
| `apps/finance/copilot-money/` | CopilotDB.sqlite | Cache-only (no API) |
