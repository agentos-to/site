...
connection("portal", base_url="https://portal.example.com",
           client="fetch",
...
> `"api"` for bare REST with key-in-header auth. Cookies ride the ambient Jar — the app
> never threads them.
>
> ```python
...
>
> # RIGHT — browser bundle rides automatically; add platform-specific headers only
...
Python clients (`requests`, `httpx`) have similar issues — `requests`/urllib3 has a blocklisted JA3 hash (`8d9f7747675e24454cd9b7ed35c58707`). Apps don't hit this because all HTTP goes through the engine's wreq client, not Python libraries directly.
...
Per-call `http2=False` is how an app opts out of HTTP/2 for a specific
request or helper:
...
Usually you wrap it once: a per-app `_get`/`_post` helper that
pins `http2=False` for every call on a Vercel-hosted platform (see
...
Vercel checkpoint. This prevented unnecessary complexity in the app code.

### You don't need `curl_cffi` or `httpx`

The engine's wreq client already emits Chrome's exact TLS cipher suites, GREASE values, extension ordering, ALPN, and HTTP/2 SETTINGS frames. Apps should never use `httpx`, `requests`, or `curl_cffi` directly — `agentos.http` handles all of this automatically.

### All I/O through SDK modules

Apps must use `agentos.http` for all HTTP — never `urllib`, `requests`, `httpx`, or `subprocess` directly. All I/O goes through SDK modules (`client.get/post`, `shell.run`, `sql.query`) so the engine can log, gate, and manage requests.
...
# Add platform-specific headers (CSRF token, Authorization,
...
`platform/sdk/python/agentos/client.py::_CHROME_VERSION` and must stay in
...
Use `bin/browse-capture.py` (CDP capture) or the fetch interceptor
...
reason. See `apps/logistics/amazon/amazon.py` for the `_get` /
...
Reference: `apps/logistics/amazon/amazon.py` `SKIP_COOKIES`.
...
**The trap:** small pages (homepages, API endpoints) may not be compressed or may use gzip which some clients handle by default. Large pages (order history, dashboards, search results) almost always use brotli. So your app works on simple endpoints and silently fails on the important ones.
...
Some platforms track request patterns and flag direct deep-links from an unknown
...
Reference: `apps/logistics/amazon/amazon.py` `_warm_session()`.
...
including 4 AppSync GraphQL calls. See `apps/media/goodreads/public_graph.py`
...
**This doesn't affect production apps.** Our architecture uses a browser
only for discovery — production calls go through `agentos.client`, which has
...
This is automatic — apps don't need to do anything. The filtering happens in the cookie provider (`brave-browser/get-cookie.py`, `firefox/firefox.py`) based on the `host` parameter the engine passes from `connection.base_url`.

**When it matters:** Only when a domain has cookies on multiple subdomains with the same cookie name. Most apps are unaffected — Amazon, Goodreads, Chase all have cookies on a single domain. Uber is the first case where it matters.

**The old workaround:** Before RFC 6265 filtering, the Uber app had a `_filter_cookies()` function that deduplicated by cookie name (last occurrence wins). This has been removed — the provider handles it correctly now.

---

## Cookie Resolution — `http.cookies()`

Apps don't resolve cookies manually — they declare a
...
async def list_trips(**params):
    ...
    # firefox / wherever. No resolver call in app code.
    return await client.post("/graphql", json={...})
```

For an app that needs to reach a second domain one-off (rare), use
...
### CDP capture integration

`bin/browse-capture.py` attaches to your real browser session, so the authed cookies are already in place — no extraction, reformatting, or injection step:

```bash
python3 bin/browse-capture.py "https://riders.uber.com/trips" --port 9222
```
...
apps/<category>/<app-name>/
  readme.md            <- AgentOS app descriptor (identity, test config)
  requirements.md      <- reverse engineering notes, API docs, findings log
  <app>.py             <- Python module with all API functions
  icon.svg             <- app icon
```
...
| App | Platform | Transport config | Key learnings |
|---|---|---|---|
| `apps/logistics/amazon/` | Amazon (Lightsaber) | `waf="cf", mode="navigate", accept="html"` | Full device hints required, `skip_cookies=` for Siege encryption, session warming. Chrome TLS fingerprint (wreq) required for orders page — Amazon's WAF uses JA4 + OpenID `max_auth_age=0` per-feature auth gates. |
| `apps/fitness/austin-boulder-project/` | Tilefive / approach.app | `accept="json"` + auth header | CloudFront, `Authorization` = namespace string |
| `apps/ai/claude/` | claude.ai (Cloudflare) | `waf="cf", accept="json"`, http2=False override | Custom Sec-CH-UA (Brave v146), Cloudflare bypass needs Sec-Fetch-* |
| `apps/web/exa/` | dashboard.exa.ai (Vercel) | `waf="vercel", accept="json"` | Vercel checkpoint is purely TLS — cookies and headers irrelevant |
| `apps/media/goodreads/` | Goodreads (CloudFront) | `waf="cf", accept="html"` | Public GraphQL via CloudFront, headless Playwright needs stealth settings |
| `apps/logistics/uber/` | Uber (CloudFront) | `accept="json"` + custom headers | RFC 6265 cookie domain filtering — first app where sibling subdomain cookies caused bugs |
