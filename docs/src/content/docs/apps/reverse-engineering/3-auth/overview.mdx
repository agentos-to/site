...
description: "How to log into things, get API keys, and store credentials — for any web platform."
---

import { CardGrid, LinkCard } from '@astrojs/starlight/components';

How to log into things, get API keys, and store credentials — for any web platform.

## In this section

<CardGrid>
  <LinkCard title="NextAuth.js / Auth.js" href="/apps/reverse-engineering/3-auth/nextauth/" description="Session cookies, CSRF double-submit, JWT vs database sessions." />
  <LinkCard title="WorkOS" href="/apps/reverse-engineering/3-auth/workos/" description="SSO flows, session cookies, API key provisioning." />
  <LinkCard title="macOS Keychain" href="/apps/reverse-engineering/3-auth/macos-keychain/" description="Reading credentials written by desktop apps via the Security framework." />
</CardGrid>
...
Every dashboard app follows these five steps.
...
# The connection — client="browser" gives us the Chrome navigate
# bundle and the ambient Jar that captures the session cookie across
# calls 1 → 4 automatically.
...
@connection("dashboard")
async def verify_login_code(*, email: str, code: str, **params):
    # 1. Trigger code email
    csrf = (await client.get(f"{AUTH_BASE}/api/auth/csrf"))["json"]["csrfToken"]
    await client.post(
    ...
    await client.get(
        ...
        f"?token={token}&email={email}&callbackUrl=..."
    )
    # → session cookie is now in the ambient Jar; at tool exit, the
    # `__cookie_delta__` sidecar persists it to the vault.
```
...
async def check_session(**params):
    # Ambient Jar is seeded from the credential row by the engine;
    # the app just makes the call.
    resp = await client.get("https://dashboard.example.com/api/auth/session")
    session = resp["json"]
...
The engine writes to the vault, creates an account entity on the
...
## Replaying with `agentos.client`

Once you understand what the browser does, replay it with
`agentos.client`. The goal is to get the same cookies without a
browser.

Apps use `agentos.client` for all HTTP — never raw
httpx/requests/urllib. Browser-grade headers (UA, `Sec-CH-UA*`,
`Sec-Fetch-*`) come from the connection's `client=` value; the
ambient Jar carries cookies. Python controls caller headers per
request via `headers={...}`; the engine's TLS fingerprint (Chrome
via wreq+BoringSSL) is a fixed match to the Python-side client hints.
...
connection("auth",
    ...
    client="browser",              # navigate bundle = form-POST bundle
    auth={"type": "cookies", "domain": ".example.com"})

@connection("auth")
async def login(*, email, password, csrf_token, **params):
    # Ambient Jar captures Set-Cookie across the redirect chain
    # automatically; the __cookie_delta__ sidecar persists at exit.
    resp = await client.post(
        "/api/auth/login",
        data={"email": email, "password": password, "csrfToken": csrf_token},
    )
    return resp["status"]
```
...
connection("api",
    ...
    client="fetch",                # XHR bundle: Sec-Fetch-Mode: cors
    auth={"type": "cookies", "domain": ".example.com"})

@connection("api")
async def login(*, email, password, **params):
...
### Picking a `client=` kind

| Kind | Bundle | Use for |
|------|--------|---------|
| `"browser"` | UA + full nav headers (`Sec-Fetch-Dest: document`, `Upgrade-Insecure-Requests`, `Accept: text/html,…`) | Form POSTs, page loads, top-level nav |
| `"fetch"` | UA + XHR headers (`Sec-Fetch-Mode: cors`, `Accept: application/json`) | AJAX/fetch calls from within a browser page |
| `"api"` (default) | No bundle — caller provides everything | Bare REST with key/token auth |

Per-request tweaks (`headers={...}` merges on top; `http2=False` for
Vercel JA4; `incognito=True` to bypass the Jar; `skip_cookies=[...]`
to drop specific cookies from the outbound header).

### When replay doesn't work

Sometimes the server does something specific to browser requests
that `agentos.client` can't replicate (custom redirect handling,
Cloudflare challenges, JS-dependent cookie setting). When that
happens:

1. **Use Playwright for that step.** Let the browser handle it.
2. **Extract the cookies** from Playwright after.
3. **Use `agentos.client` for everything else** (dashboard APIs,
   data extraction, etc.).

This isn't a workaround — it's the right architecture. Playwright
handles the login, `agentos.client` handles the work. Each tool
does what it's good at.

| Situation | Solution |
|-----------|----------|
| Standard form POST or API call | `agentos.client` replay with `client="browser"` or `"fetch"` |
| Custom OTP/code verification | Scan JS bundles for custom endpoints → `agentos.client` replay (see [discovery](../2-discovery.md#js-bundle-scanning)) |
| Google OAuth consent screen | Playwright first login → cookies → `agentos.client` after |
| Cloudflare JS challenge | Playwright or `brave-browser.cookie_get` for `cf_clearance` |
| Vercel Security Checkpoint (`429`) | `client.get(url, http2=False)` — purely a JA4 fingerprint issue |
...
# 2b. OR declare a connection with client="browser" and let the
#     engine seed the ambient Jar from the brave-browser provider
#     automatically — no manual cookie threading.
from agentos import client, connection
...
async def scrape(**params):
    return await client.get("/friend/find_friend")
```
...
### The ambient Jar handles multi-step flows automatically

When making **multi-step requests** (e.g., fetch a form page, then
submit it), the connection's ambient Jar carries Set-Cookie responses
across calls for free. No explicit jar config, no cookie header
threading.

```python
@connection("web")    # client="browser"
async def resolve_email(*, email: str, **params):
    # Step 1: CSRF token lands in the ambient Jar via Set-Cookie.
    form = await client.get("/friend/find_friend")
    n_token = _extract_n_from_html(form["body"])

    # Step 2: the same Jar ships the fresh CSRF cookie from step 1.
    # Goodreads' n= CSRF param is tied to the session cookie; if
    # step 2 sent a stale session, the server would silently ignore
    # the request. We don't have to think about any of that.
    search = await client.get(f"/friend/find_friend?n={n_token}&q={email}")
    return _parse_people(search["body"])
```

Compare to the old (now-impossible) anti-pattern: threading a raw
`Cookie:` header through every call manually, so step 1's new
`_session_id2` never rides on step 2. The Jar makes that class of
bug unrepresentable.
...
Practical notes for driving Playwright during discovery.
...
Reference implementation: `apps/web/exa/`.
...
connection("cognito",
    ...
    client="api")   # AWS Cognito is a plain REST API — no browser bundle

@connection("cognito")
async def login(*, email, password, client_id, **params):
    resp = await client.post(
        "/",
        json={
...
2. Extract cookies: `cookies { domain: ".example.com" }` — confirm
   which cookies are session-bearing.
3. Declare `connection(..., client="browser", auth={"type":
   "cookies", "domain": ".example.com"})`. The engine's
   `brave-browser` / `firefox` cookie providers seed the ambient Jar
   from the user's real logged-in browser on the first call; the
   Jar persists rotated session tokens back to the vault via the
   `__cookie_delta__` sidecar at tool exit.

Reference implementations:
- `apps/ai/claude/claude_web.py` (Cloudflare-protected, `client="fetch"` + `http2=False`)
- `apps/logistics/amazon/amazon.py` (tiered cookie architecture, Siege bypass via `skip_cookies`)

### Tiered cookie architectures

Large platforms like Amazon use multiple cookie tiers for different access levels:
...
| SSO | `sst-main` (`Sst1\|...`), `sso-state-main` | Cross-domain auth |

When building an app against a tiered platform, you need the **full cookie jar**
...
In the app's readme, mark public tools with `auth: none`:
...
Some platforms rotate API keys or endpoints when they deploy. For these, build
...
> executor reads/writes `cache` vals on the app's graph node.
> See [Data & Storage](/apps/data/).
...
Reference: `apps/media/goodreads/public_graph.py` `discover_from_bundle()`.

---

## Examples

| App | Pattern | What to learn from it |
|-------|---------|----------------------|
| `apps/logistics/amazon/` | Tiered cookie auth, Siege encryption bypass, `SESSION_EXPIRED` retry | Full client hints, cookie stripping for anti-bot, session warming, provider retry convention |
| `apps/web/exa/` | NextAuth email code → fully HTTPX (no browser) → API keys | JS bundle scanning for custom endpoints, Navigation API interception, OTP token format discovery, Vercel `http2=False` bypass |
| `apps/media/goodreads/` | Multi-tier discovery, AppSync, auth boundary mapping | Bundle extraction, config rotation, public vs auth operations |
| `apps/ai/claude/` | Cloudflare-protected cookie extraction | Stealth Playwright settings, HttpOnly cookies via CDP |
| `apps/fitness/austin-boulder-project/` | Bundle-extracted API key, tenant namespace | JS config scanning, namespace-as-auth |
