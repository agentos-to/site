---
title: "NextAuth.js (Auth.js) Pattern"
description: "NextAuth.js (rebranded to Auth.js) is the most popular auth library for Next.js apps, covering email login, Google SSO, and enterprise auth via WorkOS or similar."
---
...
[Exa app](../../reference/web/exa.md) reverse engineering session.
...
See [Discovery: JS Bundle Scanning](../2-discovery.md#js-bundle-scanning)
and [Discovery: Navigation API Interception](../2-discovery.md#navigation-api-interception)
...
`/api/auth/session`. See [Transport: http2 selection](../1-transport.md#when-to-use-http2false)
...
See [Layer 1: Transport](../1-transport.md#when-to-use-http2false)
...
this pattern in the AgentOS app library. **The entire email login flow is
...
See `apps/web/exa/exa.py` and `apps/web/exa/readme.md` for the full implementation.
...
|--------|----------|--------|
| Where it lives | In the Next.js app itself (API routes) | External auth provider |
