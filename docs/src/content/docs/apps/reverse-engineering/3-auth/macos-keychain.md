---
title: "macOS Keychain — Credential Audit & Token Extraction"
description: "The macOS Keychain is where native apps store OAuth tokens, API keys, session credentials, and encryption keys — the primary source for credentials held by desktop apps."
---

The macOS Keychain is where native apps store OAuth tokens, API keys, session
credentials, and encryption keys. For app development, it's the primary source
...
This is exactly how the `mimestream.credential_get` tool works — `command:` step
...
a malicious app), it can silently read any non-ACL Keychain item:

```bash
# A malicious app could shell out to do:
...
**Implication for AgentOS:** Apps that call `shell.run(...)` can execute
arbitrary shell commands. Before a public app registry exists, every use of
`shell.run` in community apps should be audited for Keychain access. App
sandboxing is a backlog item.
...
- `apps/comms/mimestream/` — reference
