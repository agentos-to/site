...
description: "Apps declare their external platform dependencies as module-level connection() calls in Python. Each connection carries base_url, auth, optional description, label, help_url, and local data sources."
---

A **connection** is how an app reaches an external platform (HTTP API, OAuth-
protected endpoint) or a local data source (SQLite file, app cache). Every
app declares its connections at **module level** in Python. There is no
`connections:` YAML frontmatter block — the engine reads Python.

## The shape

```python
# apps/your-app/your_app.py
...
on the app. Tools bind to a connection with the `@connection("name")`
decorator. Apps with exactly one connection auto-infer; tools on
multi-connection apps must tag the connection they use.
...
required. The key is stored with `apps.connect({ app, key })`
(MCP/CLI/HTTP), which writes the encrypted vault row + account node
and returns the connection's state. `apps.load` shows per-connection
...
**Public + authed** — two identities against the same platform:
...
When connections differ by transport rather than platform, a tool can name
...
- Single-connection apps auto-infer: no `@connection` decorator needed.
- Multi-connection apps must decorate every public tool.
- A tool that should skip auth entirely uses `@connection("none")` (or omits
  auth by virtue of its connection having no `auth=` dict).
- `optional=True` on a connection means the app works anonymously but
  improves with credentials.
- Connections without any auth (just `base_url` / `sqlite` / `vars`) are
  valid — they serve as platform declarations.
...
stored sessions) or provider apps (Brave, Firefox):
...
Apps that are purely local (no external platforms) declare their
...
**not** on `(app, connection_name)`. The domain comes from the
connection's `base_url` (or an explicit `domain=` override):

| App | Connection | `base_url` | Derived domain |
|---|---|---|---|
...
Two consequences worth understanding as an app author:

- **Connection names are local labels.** Twenty apps can each
...
- **Reorganizing your app never loses cookies.** Move the app
...
`base_url` derivation — useful when a platform uses subdomains that
...
At first dispatch of any tool in the app, the engine walks the
app's `.py` files, finds every module-level `connection(...)` call,
and extracts the literal kwargs into the app's in-memory
...
Secrets belong in the vault, not in source; apps
