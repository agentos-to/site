...
- **Privacy by design** — your messages, tasks, and contacts never leave your computer unless an app you installed specifically sends them somewhere.
...
- **Encrypted credentials** — sessions for platforms you've connected. Keys in Keychain, values in that same SQLite file.
- **App code** — the Python that runs when you invoke an app. Read at call time from `~/dev/agentos/apps/` or wherever you installed the app.
...
- **No "freemium."** The same binary does the same things for everyone.
...
See [Security](/architecture/security/) for why the engine's brokering architecture matters for this — and why apps can't exfiltrate data they don't have permission to touch.
