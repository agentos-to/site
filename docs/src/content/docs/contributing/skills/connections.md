...
- With `connection: [a, b, …]`, the first entry is the default; expose `connection` in `params` and pass it through from Python so the runtime resolves the effective connection
...
When a Python function receives cookies via `params.auth.cookies`, the value is a **cookie header string** — e.g. `"name1=val1; name2=val2"`. This is the same format as the HTTP `Cookie` header.
...
Example references (readme frontmatter + `.py`):

- OAuth consumer: `skills/comms/gmail/`
- OAuth provider: `skills/comms/mimestream/`
- Cookie consumer: `skills/ai/claude/`
- Cookie provider (browser DB): `skills/browsers/brave/`
- Multi-connection: `skills/ai/claude/` (web + cli + api)
