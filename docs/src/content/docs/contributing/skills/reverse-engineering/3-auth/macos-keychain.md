...
```bash
# A malicious skill could shell out to do:
...
**Implication for AgentOS:** Skills that call `shell.run(...)` can execute
arbitrary shell commands. Before a public skill registry exists, every use of
`shell.run` in community skills should be audited for Keychain access. Skill
