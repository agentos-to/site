...
| [HTTP](/interfaces/http/) | The desktop shell | Localhost HTTP/SSE on `127.0.0.1:3456` |
| GUI | End users | The desktop shell on the HTTP bridge, browsed via Tauri or a standard browser |

All four converge on the same engine. The engine doesn't know which interface made a call — it sees JSON-RPC requests and dispatches to apps. Interfaces are transport; the engine is logic.

## Pick one

- **You're building an app** — use the **CLI** to test it (`agentos call apps '{"op":"run",...}'`) and the **MCP** interface to see how an AI client would invoke it.
- **You're writing an AI-client integration** — **MCP** is the protocol. Everything else is implementation detail.
- **You're working on the shell** — talk to the **HTTP** bridge. Don't try to connect to the engine socket directly.
- **You just want to use AgentOS** — launch the **GUI** (the desktop shell).
...
- Browsers can't speak Unix sockets, so a **localhost HTTP** bridge translates. The shell gets a regular fetch-based API; the engine stays in its own process.
- The **GUI** is how end users actually see the thing. The engine itself serves a web server; you browse it through Tauri (native window) or any standard browser (`http://localhost:3456`). It's the OS-like window into your graph — inspect entities, run [apps](/apps/overview/) in their generated windows. Optional: the CLI and engine work fully without it.
