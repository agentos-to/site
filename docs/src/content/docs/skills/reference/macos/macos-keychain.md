...
description: "Credential provider backed by the macOS login Keychain. Exposes internet-password entries matching a domain via `@provides(login_credentials)` so skills' `lo..."
sidebar:
...
- [`credential`](/shapes/reference/credential/) — from `get_credentials`
