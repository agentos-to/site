...
<CardGrid>
  <LinkCard title="NextAuth.js / Auth.js" href="/skills/reverse-engineering/3-auth/nextauth/" description="Session cookies, CSRF double-submit, JWT vs database sessions." />
  <LinkCard title="WorkOS" href="/skills/reverse-engineering/3-auth/workos/" description="SSO flows, session cookies, API key provisioning." />
  <LinkCard title="macOS Keychain" href="/skills/reverse-engineering/3-auth/macos-keychain/" description="Reading credentials written by desktop apps via the Security framework." />
</CardGrid>
...
Skills use `agentos.client` for all HTTP — never raw
...
- `skills/ai/claude/claude_web.py` (Cloudflare-protected, `client="fetch"` + `http2=False`)
- `skills/logistics/amazon/amazon.py` (tiered cookie architecture, Siege bypass via `skip_cookies`)
