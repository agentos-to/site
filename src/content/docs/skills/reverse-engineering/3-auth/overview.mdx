...
async def verify_login_code(*, email: str, code: str, **params):
    ...
    # `__cookie_delta__` sidecar persists it to the credential store.
```
...
3. Declare `connection(..., client="browser", auth={"type":
   ...
   Jar persists rotated session tokens back to the credential store
   via the `__cookie_delta__` sidecar at tool exit.
